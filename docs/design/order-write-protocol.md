...
without credential acquisition or a send. Generated bytes are discarded. A local refusal is `invalid_input`;
the actual client still validates its request and credentials at the send boundary.
