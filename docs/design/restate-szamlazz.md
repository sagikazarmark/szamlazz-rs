...
is a replace on a document the worker did not issue. The remedy is the caller's, stated in both READMEs: serialise
...
run-name pin** — `RUN_NAMES` in the e2e harness (`tests/service.rs`), verified against a live `sys_journal` on every
run (§11) — and this table follows it: a step added, renamed or reordered in the code fails the pin first, and the
table is then brought to match, never the other way round. The names are what the Restate UI shows, what a
`sys_invocation.last_failure_related_command_name` names, and what an `unavailable` fault's message means by "the
step". A `{kind}` is one of `proforma | invoice | prepayment | final | corrective`; `{number}` the caller's invoice
number as sent; `{prefix}` the eight-digit taxpayer prefix. A handler with two shapes has two rows; a handler that
...
reply is `outcome_unknown`. `hint-storno-{number}` is the one read whose exhaustion is not a fault — the best-effort
storno-number hint (§6 step 1) — and `Szamlazz.Agent.storno`'s `lookup-storno-{number}` after a verify that saw the
document reversed is best-effort in the same way (§4, its row). The parametrized names are read by their prefix —
`verify-storno-…` is `verify-storno-{number}`, never `verify-{number}` — which is why the pin needs no invoice number
to itself begin with a pinned stem (none of szamlazz.hu's do).
