...
[operator runbook](../operations/order-recovery.md) is the current recovery and effective-controls reference.
...
- `service::journal` (what a `ctx.run` result may hold; ADR 0009): a sample of every variant of every type the
  ...
  `buyer`, `items`, `financial_items`, `labels` or `pdf` key at any depth after checking and excluding the explicit
  query's three-field buyer identity allowlist. Explicit queries journal `QueryOutcome<QueryResponse>` with
  buyer identity and per-VAT rows; mutation, reconciliation and Order reads retain reduced `FoundDocument`
  values (the agent crate's own `InvoiceDocument`
