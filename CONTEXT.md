...
**Create step**:
The second durable step of issuing (`create-{kind}`): one `ctx.run` under the issue policy whose every execution is query-first *inside the closure* — the external-id query first, the send only when the external id holds **nothing**, or **exactly the document the lookup step saw reversed**, still reversed. A live document that is not the reversed one the lookup saw was issued by an earlier execution and is answered as `issued` without sending; a reversed document that is not the one the lookup saw was issued by an earlier execution and reversed since, and is answered as `reversed` without sending (a reversal the caller has not acknowledged — *Reissue* is always explicit); the lookup's reversed document reported live is a server inconsistency answered as `conflict{live}` without sending. A lost reply is re-queried once, immediately; nothing found makes the step **unconfirmed**, and Restate re-executes it after the policy's delay. The query lives inside the closure because a separate journaled pre-query would replay its stale "nothing" on the retry. Gateway fn: `Gateway::create` → `Result<CreateOutcome, Unconfirmed>`.
_Avoid_: attempt, issue step (the whole two-step sequence is "issuing"), send (one part of the step)

**Unconfirmed**:
The one error of the create and storno steps: szamlazz.hu's answer is not known — a transport failure or an open code (1, 55, 56 without a number, `szlahu_down`) — after the immediate re-query found nothing. Retryable to the SDK, so the issue policy re-executes the step; its exhaustion is the `outcome_unknown` fault. Every *known* answer, including rejections, duplicates and the duplicate-order-number contradiction (71/152 with nothing under the order — a refusal, settled after one send), is `Ok` data.
_Avoid_: unknown outcome (the fault is `outcome_unknown`; this is what precedes it), transport error (one cause of it), `Contradiction` as a variant (the pre-#41 retryable one; a 71/152 with nothing under the order is settled data now)

**Issue policy**:
The run retry policy of the create and storno steps: `[issue]` in the deployment configuration — `max_attempts` executions, `initial_delay` growing by `factor` to `max_delay`, bounded by `max_duration` — mapped field for field onto `RunRetryPolicy::new()`. Deployment-level (on `WorkerConfig`), shapes no journal entry. Set explicitly because the SDK's default run policy sends no retry delay and the server would spend the handler's `invocation_retry_policy` instead. `max_duration` is the hard bound; the attempt count is not durable across every replay (ADR 0004). One of the three policies of the deployment, beside the *Read policy* and the *Resolve policy*. Type: `restate_szamlazz::config::IssueConfig`.
_Avoid_: attempt budget, backoff (the loop is gone), retry policy without qualification (the handlers have their own)

**Read policy**:
The run retry policy of every read-only durable step of both services — the *Lookup step*, the exclusivity, proforma-link and `get` lookups, the verifies, the order-number hint, the storno lookup, `Szamlazz.Agent.query` and the *check_account* probe: `[read]` in the deployment configuration, the same five fields as the *Issue policy* (`max_attempts`, `initial_delay`, `factor`, `max_delay`, `max_duration`; defaults `3`, `5s → 30s`, `2m`) mapped field for field onto `RunRetryPolicy::new()`. A read szamlazz.hu did not answer is the step's *Unanswered* error, re-executed under it; every szamlazz.hu *answer* — a document, code 7, a credential code, another API code — is data and never retried. Its exhaustion (or a cancel mid-read) is the `unavailable` fault, naming the step and the last failure, about the document when the step knows one. Safe because a read writes nothing and a re-executed closure's answer is exactly as fresh as a first one; sized short because szamlazz.hu is observed to stall for a minute at a time. Deployment-level (on `WorkerConfig`), shapes no journal entry; set explicitly for the same reason as the issue policy. The one read that does not fault on exhaustion is the storno-number hint after a verify found the document already reversed — best effort, `None`. Decision: ADR 0004 (#37 amendment). Type: `restate_szamlazz::config::ReadConfig`.
...
**Unanswered**:
The one error of every read fn of the *Gateway* (`lookup`, `verify`, `query`, `hint`, `lookup_storno`, `probe`): szamlazz.hu produced no answer — a transport or parse failure (`Transport`) or `szlahu_down` (`Unavailable`). The read-side twin of *Unconfirmed*: retryable to the SDK, so the *Read policy* re-executes the step; its exhaustion is the `unavailable` fault. Never a journaled outcome — the `Transport` variants of `LookupOutcome`, `QueryOutcome`, `StornoLookupOutcome` and `ProbeOutcome` are gone; another szamlazz.hu code on a read is the data variant `Api{code, message}`, which the handlers that cannot conclude from it answer as `unavailable` without a retry (`Szamlazz.Agent.query` passes it through as 422; the probe accepts it). Type: `restate_szamlazz::gateway::Unanswered`.
