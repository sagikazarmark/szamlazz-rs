...
**Prologue**:
The four steps every handler of both Restate services runs after parsing its key and before its operation: **pin** the namespace in a pure durable step (`namespace`); **resolve** the request's scope to its *Account* in a durable step named `account` under the *Resolve policy* — unscoped and unknown journaled as data and answered as `unknown_account`, an unavailable resolver retried, its exhaustion `unavailable`; **fetch** the account's credentials outside the journal on every execution with a short in-process retry, then terminal `unavailable`; **open** the *Gateway* over a fresh client. The ingress-path guard (#27) slots in before the resolve. Its decisions are pure functions (`service::prologue`); its durable behaviour is observable only under Restate. Helper: `support::{object, shared, service}::prologue`.
