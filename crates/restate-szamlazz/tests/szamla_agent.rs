//! Wiremock tests of the low-level `szamla_agent` layer: the issue closure
...
use restate_szamlazz::szamla_agent::{
    DeleteOutcome, DocumentRefs, IssueOutcome, IssueRequest, QueryError, QueryOutcome,
    SetPaymentsOutcome, StornoAttempt, StornoOutcome, SzamlaAgent,
};
...
fn agent(server: &MockServer) -> SzamlaAgent {
...
async fn verify_query_and_hint() {
    ...
    match h.agent.verify("SZ-1").await {
    ...
    assert_eq!(h.agent.verify("SZ-404").await, QueryOutcome::NotFound);
    assert!(matches!(
        h.agent.verify("SZ-500").await,
        QueryOutcome::Transport(_)
    ));
    match h.agent.hint(&order()).await {
        QueryOutcome::Found(found) => {
...
async fn storno_reversed_is_validated() {
    ...
    assert_eq!(
        h.agent.storno(storno_attempt(&storno_id)).await,
...
async fn delete_proforma_outcomes() {
    ...
    assert_eq!(h.agent.delete_proforma("D-1").await, DeleteOutcome::Deleted);
    assert_eq!(
        h.agent.delete_proforma("D-2").await,
        DeleteOutcome::AlreadyGone
    );
    assert_eq!(
        h.agent.delete_proforma("D-3").await,
    ...
    assert!(matches!(
        h.agent.delete_proforma("D-4").await,
...
async fn set_payments_outcomes() {
    ...
    let six = vec![entry; 6];
    assert!(matches!(
        h.agent.set_payments("SZ-9", &six, false).await,
