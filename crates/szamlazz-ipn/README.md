...
Everything else is **content**. `gross_total`, `paid_gross`, `payment_date` and `payment_method` are optional. Missing or empty values become `None`, as do unreadable dates and amounts. Dates are read only as `%Y-%m-%d`; a datetime remains available as raw text. Amounts are trimmed and accept an optional sign, ASCII digits, and a dot or a lone comma as the decimal separator (including `.5` and `1.`). Exponents are unreadable form content. The previous `Decimal::from_str_exact` underscore tolerance after a digit is retained. Parsing is iterative, so arbitrarily many leading zeroes do not exhaust the stack or make an otherwise representable amount unknown. Excess precision (more than 28 fractional digits, even zeroes) or range becomes `None`, never silently rounded. Unknown non-empty payment methods remain strings.
...
JSON significands also use iterative parsing. Redundant fractional zeroes may be removed to obtain an exact value; unlike form content, `0.000…` with more than 28 fractional zeroes is accepted as zero. Representable scale is preserved. JSON's leading-zero rule still applies to numeric strings (`"0012"` is rejected).
