...
The `restate-szamlazz` binary serves the `Order` Virtual Object and the `SzamlaAgent` service of [`restate-szamlazz`](../restate-szamlazz) over HTTP/2 for a Restate server to register. It issues, reverses and reconciles szamlazz.hu documents exactly once per order; the design is in [`docs/design/restate-szamlazz.md`](../../docs/design/restate-szamlazz.md).
...
One deployment serves one szamlazz.hu account. A second account is a second deployment with its own `Order` service.
...
`Order` is a Virtual Object keyed by the order number (`rendelésszám`, trimmed). Every issuing handler takes a `request_id`: the retry identity. The same id returns the entry's current state forever; a different id is a new logical request; a known id with a different payload is `conflict{payload_mismatch}`.

| Handler | Description |
|---|---|
| `Order.create_proforma` | Issues the proforma (`díjbekérő`) of the order. |
| `Order.create_invoice` | Issues the invoice (`számla`), optionally converting the order's proforma (`options.proforma`). |
| `Order.create_prepayment` | Issues the prepayment invoice (`előlegszámla`); one per order. |
| `Order.create_final` | Issues the final invoice (`végszámla`) settling the committed prepayment. |
| `Order.correct_invoice` | Issues a corrective invoice (`helyesbítő számla`) for an invoice managed by this order; a new `request_id` issues a new corrective. |
| `Order.storno_invoice` | Reverses (`sztornó`) an invoice managed by this order; idempotent. |
| `Order.delete_proforma` | Deletes the order's proforma; refuses a paid one unless `force`. |
| `Order.get` | The order's ledger as recorded, or with `verify` after checking every committed document against szamlazz.hu. Read-only, never blocks behind issuing. |
| `Order.record_reversal` | Operator assertion that a recorded document is reversed or live (private: not reachable from the ingress). |
| `Order.forget` | Operator drop of a slot whose document szamlazz.hu no longer knows (private). |
| `SzamlaAgent.query` | Queries a document by invoice number, order number or external id. |
| `SzamlaAgent.set_payments` | Registers credit entries (`jóváírás`) on an invoice; replaces unless `additive`. |
| `SzamlaAgent.storno` | Reverses an invoice that no `Order` manages; a document carrying an order number is answered with `managed_by_order` instead. |

A create request through the ingress:

```sh
curl localhost:8080/Order/ORD-1001/create_invoice \
