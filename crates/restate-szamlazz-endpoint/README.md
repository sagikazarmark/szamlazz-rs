...
The `restate-szamlazz` binary serves the `Szamlazz.Order` Virtual Object and the `Szamlazz.Agent` service of [`restate-szamlazz`](../restate-szamlazz) over HTTP/2 for a Restate server to register. It issues and reverses szamlazz.hu documents exactly once per order, keeping no state of its own: szamlazz.hu is the source of truth, reached through deterministic external ids.
...
The Restate UI at <http://localhost:9070> lists the two services and every invocation with its journal; it is the debugging tool for everything below.
...
Caused by:
    unknown field: found `endpont`, expected `one of `id`, `agent_key`, `endpoint`, `defaults`, `seller`` for key "account.endpont" in restate-szamlazz.toml TOML file
```

An environment override renders as the variable that set it (`for key "RESTATE_SZAMLAZZ_ISUE" in environment variables`). The first unknown key is reported; fix it and run `--check-config` again. A value of the wrong type is refused the same way, naming the key and the source (`invalid type: found string "three", expected u32 for key "RESTATE_SZAMLAZZ_ISSUE__MAX_ATTEMPTS" in environment variables`); `[account]` together with a non-empty `[accounts]` names both tables and where each came from (the case of a stray `RESTATE_SZAMLAZZ_ACCOUNT__AGENT_KEY` left over after the [flag day](#single--multi-flag-day), which would otherwise materialise a partial `[account]`). Then the invariants are checked (`WorkerConfig::validate`, the static resolver's account rules) and the process exits with the first violated one. One of them is a floor rather than a shape: `issue.initial_delay` must be at least 90 s (the Számla Agent client's 60 s request timeout plus a 30 s margin), because a create or storno step that is re-executed sooner would query for the cut execution's send while that send may still be in flight (szamlazz.hu has been seen to stall for a minute and still issue). `initial_delay = "5s"` is refused with `issue.initial_delay (5s) must be at least 90s: …`, naming the rule; `[read]` has no floor because a read writes nothing, `[resolve]` none because it never reaches szamlazz.hu.
...
`[account]` and `[accounts.<scope>]` are mutually exclusive: both present is a load error, and there is no default account. In this shape an **unscoped** request is `unknown_account` (400); in the single-account shape a **scoped** one is. The configuration is validated at start-up against the checkable half of the safety contract (one szamlazz.hu account is reachable under exactly one scope), and the process exits on: two accounts sharing an `(endpoint, agent_key)` pair (the endpoint compared normalised: `https://www.szamlazz.hu/szamla/` and `https://www.szamlazz.hu/szamla`, or `HTTPS://…`, or `:443` written out, are one endpoint, so a defaulted and a typed endpoint with one key are caught), two sharing an `id` (the credential reference), or a scope key outside `[a-z0-9_]` / longer than 36 bytes.
...
| `503 unavailable` | szamlazz.hu did not answer the probe through the `[read]` policy, or the resolver or the credential store could not be reached (or did not answer within the worker's ten-second bound on the call); call again. |
...
**Rolling updates.** To roll without cutting anything, empty the server side first (steps 1 and 2 of the [flag-day script](#single--multi-flag-day): make both services private and poll the `sys_invocation` query until nothing is in flight), then stop the old revision, register the new one and make the services public again. Without it a rollout is correct but stalls every in-flight order for the retry interval and spends one of its attempts. It is *correct* because every type the worker journals as a `ctx.run` result is crate-owned, additive-only and pinned by fixtures in CI (see *Journal compatibility* in the library README): an invocation the new revision resumes decodes the entries the old one wrote. The drain is what avoids the stall, not what keeps the invocations alive; a release that deliberately breaks a journaled shape says so in its notes, and for that one the drain is mandatory. **The first release after #127 is that release**: the document outcomes changed shape (the worker's own `FoundDocument` / `IssuedDocument` projections replaced the Számla Agent crate's `InvoiceDocument` / `InvoiceCreationResult` / `CreatedInvoice` in the journal; ADR 0005, #127 amendment), so an invocation of an earlier revision that is resumed by it fails to decode any entry that carries a document (`lookup-*`, `create-*`, `exclusivity-*`, `prepayment-for-final`, `proforma-link`, `proforma-for-delete`, `verify-*`, `hint-storno-*`, `get-*`, `query`, `storno-*`) and is killed once its attempts are spent. The break was taken before the first production deployment, where nothing was in flight; a deployment that did run an earlier revision drains before registering this one.
...
| `Szamlazz.Order.create_prepayment` | Issues the prepayment invoice (`előlegszámla`); one per order, exclusive with the plain invoice. Converts the order's live proforma unless told otherwise (`options.proforma`, exactly as `create_invoice` takes it); `options.reissue` issues a new one after a reversal. |
| `Szamlazz.Order.create_final` | Issues the final invoice (`végszámla`) settling the order's live prepayment invoice. szamlazz.hu links the prepayment invoice but does **not** net it into the totals: the `document` you send lists the full performance and deducts the prepayment as a negative line at the same VAT rate (verified on the test account). Takes no `options.proforma`. |
...
| `Szamlazz.Order.storno_invoice` | Reverses (`sztornó`) an invoice of this order; idempotent. The storno carries the original's fulfillment date (`teljesitesDatum` = the invoice's `telj`), as NAV requires; there is no way to set another. |
...
| `Szamlazz.Agent.set_payments` | Registers credit entries (`jóváírás`) on an invoice; replaces unless `additive`. At most five entries: a sixth is 400 `invalid_input`, nothing sent; szamlazz.hu refusing the entries is 422 `szamlazz_error`. No query precedes the send, so it has no `not_found` path. **`additive: true` is at-least-once**: a lost reply is `outcome_unknown`, and the handler's one retry after a crash re-sends; each send that reaches szamlazz.hu appends the entries again. Query the invoice before re-sending an additive call that ended in `outcome_unknown`; a replacing call is repeated as is. **Not serialised per invoice**: two concurrent replacing calls race and the last send to land wins ([`Szamlazz.Agent` is unkeyed](#szamlazz-agent-is-unkeyed)). |
| `Szamlazz.Agent.storno` | Reverses an invoice that no `Szamlazz.Order` manages; a document carrying an order number is answered with `managed_by_order` instead: read off the verified document, like every document found by number; an unknown invoice number is 404 `not_found`; szamlazz.hu refusing the storno is `outcome: rejected` (200), never a 422. The storno carries the original's fulfillment date, like `storno_invoice`. Not serialised per invoice either ([below](#szamlazz-agent-is-unkeyed)). |

<a id="szamlazz-agent-is-unkeyed"></a>**`Szamlazz.Agent` is unkeyed.** It is a stateless Restate service, so its invocations run concurrently, unlike `Szamlazz.Order`, whose per-key lock runs one handler per order at a time. Two by-number writes on the same invoice are therefore not ordered by the worker: two `set_payments` with `additive: false` both send their full replacement, and whichever szamlazz.hu applies last is the invoice's payment state (under reordered webhook deliveries, the older snapshot); two `storno`s of one invoice both send, and szamlazz.hu's idempotent storno answers the repeat with the existing storno number (verified for a sequential repeat; two sends in the same instant are unverified). A keyed `Szamlazz.Document` Virtual Object per invoice number was considered and judged over-engineering for two writes whose only hazard is a replace. The remedy is the caller's: serialise `set_payments` per invoice on your side (a queue keyed by the invoice number, or a lock in the webhook handler), or send `additive: true` and let szamlazz.hu sum the entries, which no ordering can corrupt.
...
| `options` | `reissue` (default `false`): issue a new document after the existing one was reversed; on a live document it is `conflict{live}`. `proforma` (`create_invoice` and `create_prepayment`; default `"auto"`): `"auto"` converts the order's live proforma if there is one, `"none"` refuses to (`conflict{proforma_live}` while one is live), `{"number": "D-…"}` names one. `create_proforma` and `create_final` refuse anything but `"auto"`. |
...
{
  "code": 503,
  "message": "{\"code\":\"credentials_rejected\",\"message\":\"szamlazz.hu rejected the agent credentials (code 3: Sikertelen bejelentkezés); the outcome is not known; fix the account's agent key, then retry with a new Idempotency-Key or read get\",\"szamlazz_code\":\"3\",\"order\":\"ORD-1001\",\"kind\":\"invoice\",\"external_id\":\"acct:ORD-1001:invoice\"}",
  "source": "invocation"
}
```
...
Two more bodies wear the same envelope and are **not** the worker's fault. A **killed invocation** (the handler's invocation attempts exhausted by a worker outage) is a 500 with `source: invocation` whose `message` is Restate's description of the last retryable error, **text, not JSON** (the wording is Restate's and varies); treat it as `outcome_unknown`:
...
|---|---|---|---|
| `invalid_input` | 400 | The request is malformed: its body carries a field the contract does not know, a wrong type, a missing required field, an `invoice_number` or `correction_id` outside its bound, or an order key with leading or trailing whitespace or outside the key alphabet (1–40 bytes, no control character, no internal whitespace, no `:`, NFC; the message names the rule; nothing was journaled or sent), or it carries a value the operation cannot take: an option the handler does not take (`options.proforma` on `create_proforma` or `create_final`), a `{number}` proforma link that is not a proforma, an empty `buyer.name`, a `query_taxpayer` tax number in neither accepted form (refused before anything is journaled, like a malformed body), a sixth credit entry on `set_payments` or a replacing one with no entries (the wire contract takes five, and an empty replace would clear the invoice's payments; nothing is sent), a line item whose arithmetic overflows a decimal (nothing is sent). | Fix the request. |
