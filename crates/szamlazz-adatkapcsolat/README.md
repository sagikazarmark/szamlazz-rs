...
Fixed-key `axum::router` and `axum::router_with_body_limit` require a non-empty configured key and panic **at construction** if it is empty. Keys are compared as supplied, without trimming. An empty key presented in a request is an unknown key under a non-empty fixed configuration and receives the normal `KEY_ERR` Ack; it never triggers the construction check.
...
Decimal fields validate the whole numeric token before conversion: optional sign, ASCII digits with an optional decimal point, and optional signed `e`/`E` exponent. Surrounding whitespace and spellings such as `.5`, `1.` and leading zeroes remain accepted; underscores, comma/grouping separators, non-finite tokens and malformed suffixes are shape errors. Missing or empty elements remain `None`. Conversion retains `Decimal`'s existing range and rounding policy. Redundant leading integer zeroes are removed before conversion so their length cannot exhaust the parser's stack; the original XML is retained.
...
XML shape means one complete XML 1.0 document: exactly one root, no leading or trailing non-whitespace text, and legal XML characters and references even in ignored elements and attributes. Comments, processing instructions, CDATA inside elements, and unknown elements in the document's namespace remain accepted. Namespace declarations use XML attribute normalization: `http:&#47;&#47;` and `http://` identify the same namespace, on the root and on children; prefix bindings and attribute-name uniqueness are checked after normalization. A document type declaration may be present before the root. Its internal subset's declaration grammar, quoted values, character references, comments and processing instructions are checked, but no external DTD is fetched, declared entities (including parameter entities) are not expanded, and DTD defaults and validity constraints are not applied to the record. Parameter references between declarations are checked lexically only; their replacement text is not interpreted. The original body remains available byte-for-byte through `raw_xml()`.
...
Without the router, `Document::identify` names the pushed kind (`RootKind`) from the root start tag and its normalized namespace only (UTF-8 is checked over the whole body). The full XML shape check belongs to `Document::parse`, after authentication in the router. A receiver can follow that same order (identify, authenticate, then parse) and answer an unknown key with `ControlCode::KeyUnknown.to_xml(kind)` without parsing the document content.
...
- Decimal XML fields reject non-numeric lexical extensions previously accepted by `Decimal::from_str`, such as `1__2`, including malformed suffixes beyond its rounding point. Fixed-key router construction now panics for an empty configured key.
